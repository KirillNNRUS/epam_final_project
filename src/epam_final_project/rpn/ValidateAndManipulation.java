    ...
    private List<String> notValidParenthesis = new ArrayList<String>() {
        {
            ...
            //Для 3(-25)
            this.add("^[)].+[(]$");
            //Для [НачалоСтроки])ВсеЧтоУгодно([КонецСтроки]
            this.add("^[)].+[(].+");
            //Для [НачалоСтроки])ВсеЧтоУгодно(ВсеЧтоУгодно
            this.add("^.+[)].+[(]$");
            //ВсеЧтоУгодно)ВсеЧтоУгодно([КонецСтроки]
            //TODO последние 2-е переделать, в просто, если скобка с начале или в конце Строки - исключение
        }
    };
